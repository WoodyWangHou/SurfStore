...
public final class Client {
    ...
    protected List<Integer> getVersion(String fileName){
        ...
        FileInfo req = FileInfoBuilder.toFileInfo(fileName, 0, null, false);
    ...
    protected boolean delete(String fileName) throws NoSuchFileException{
        ...
        FileInfo req = FileInfoBuilder.toFileInfo(fileName, version + 1, null, true);
    ...
    private void serve(Namespace args){
      ...
      private static List<Block> getFileBlockList(String localPath){
        ...
        try{
            ...
            while (fc.read(buf) > 0) {
                byte[] data = buf.array();
                Block bl = BlockBuilder.bytesToBlock(data);
                res.add(bl);
                buf.flip();
              }
              fc.close();
      ...
      private WriteResult updateMetaStore(Path localPath, int version){
          ...
          FileInfo req = FileInfoBuilder.toFileInfo(fileName, version, hashList, false);
